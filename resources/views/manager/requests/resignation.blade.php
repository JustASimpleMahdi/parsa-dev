@php use App\RequestStatusEnum; @endphp
    <!DOCTYPE html>
...
<div class="card">
    <form action="{{ route('manager.requests.resign',['request' => $request]) }}" method="post" class="card-content">
        @csrf
        @method('DELETE')
        ...
        <label>
            <input name="response" class="checkbox" type="radio" value="accept">
            تایید استعفا
        </label>
        <label>
            <input name="response" class="checkbox" type="radio" value="reject" checked>
            رد استعفا
        </label>

        <div id="reject" class="response-box show">
            ...
            <textarea name="text" type="text"
                      class="editable-input">{{ old('text',$request->response?->text) }}</textarea>

            <!-- دکمه ارسال با اندازه کوچک‌تر -->
            <div class="button-wrapper">
                <button @disabled($request->status === RequestStatusEnum::RESPONDED) class="submit-btn">رد درخواست
                </button>
            </div>
        </div>
        <div id="accept" class="response-box">
            <div class="button-wrapper">
                <button @disabled($request->status === RequestStatusEnum::RESPONDED) class="submit-btn">تایید استعفا
                </button>
            </div>
        </div>
    </form>
...
<script>
    ...
    checkboxes.forEach((el) => {
        el.addEventListener('input', () => {
            ...
            divs[el.value].classList.add('show')
        });
    })
</script>
</body>
